# 工具集（ToolSet）架構設計

## 設計理念

### ❌ 舊架構的問題
```python
# Prompt 硬編碼工具列表
user_prompt = f"""
請使用以下工具：
1. tej.stock_price - 查詢股價
2. searxng.search - 網頁搜尋
"""
```

**問題**：
- Agent 被強制使用特定工具
- 失去自主選擇能力
- 無法靈活管理工具權限
- 難以擴展和維護

### ✅ 新架構：工具集系統

```
┌─────────────┐
│   ToolSet   │  工具集 = 工具的集合
└──────┬──────┘
       │
       ├─ Tool 1: tej.stock_price
       ├─ Tool 2: tej.company_info
       └─ Tool 3: searxng.search

┌─────────────┐
│    Agent    │  Agent 被分配工具集
└──────┬──────┘
       │
       ├─ ToolSet A (assigned)
       ├─ ToolSet B (assigned)
       └─ Global ToolSet (自動)
```

**優勢**：
- ✅ Agent 自主決定使用哪些工具
- ✅ 靈活的權限管理
- ✅ 可重用的工具集
- ✅ 前端可視化管理

---

## 資料庫設計

### 1. ToolSet 表
```sql
CREATE TABLE toolsets (
    id TEXT PRIMARY KEY,              -- UUID
    name TEXT NOT NULL,               -- 工具集名稱
    description TEXT,                 -- 描述
    tool_names JSON NOT NULL,         -- ["tej.stock_price", "searxng.search"]
    is_global BOOLEAN DEFAULT FALSE,  -- 是否為全局工具集
    created_at TIMESTAMP,
    updated_at TIMESTAMP
);
```

### 2. AgentToolSet 關聯表（多對多）
```sql
CREATE TABLE agent_toolsets (
    id TEXT PRIMARY KEY,
    agent_id TEXT NOT NULL,    -- FK to agents.id
    toolset_id TEXT NOT NULL,  -- FK to toolsets.id
    created_at TIMESTAMP
);
```

---

## 工具集類型

### 1. 全局工具集（Global ToolSet）
- **自動分配**給所有 Agent
- 包含所有已註冊的工具
- 系統自動維護

```python
{
    "name": "全局工具集",
    "description": "包含所有已註冊的工具",
    "tool_names": [
        "tej.stock_price",
        "tej.company_info",
        "searxng.search",
        "yfinance.stock_info",
        ...  # 所有 25 個 TEJ 工具
    ],
    "is_global": True
}
```

### 2. 專用工具集（Specialized ToolSet）
- **手動分配**給特定 Agent
- 針對特定任務設計

**範例 1：台股分析工具集**
```python
{
    "name": "台股分析工具集",
    "description": "專門用於台灣股市分析",
    "tool_names": [
        "tej.stock_price",
        "tej.company_info",
        "tej.monthly_revenue",
        "tej.institutional_holdings",
        "tej.margin_trading",
        "tej.financial_summary"
    ],
    "is_global": False
}
```

**範例 2：網頁搜尋工具集**
```python
{
    "name": "網頁搜尋工具集",
    "description": "用於網頁搜尋和資訊收集",
    "tool_names": [
        "searxng.search",
        "duckduckgo.search"
    ],
    "is_global": False
}
```

---

## API 設計

### ToolSet 管理

#### 1. 創建工具集
```http
POST /api/v1/toolsets
Content-Type: application/json

{
    "name": "台股分析工具集",
    "description": "專門用於台灣股市分析",
    "tool_names": [
        "tej.stock_price",
        "tej.company_info"
    ],
    "is_global": false
}
```

#### 2. 列出所有工具集
```http
GET /api/v1/toolsets
GET /api/v1/toolsets?is_global=true
```

#### 3. 獲取工具集詳情（包含工具列表）
```http
GET /api/v1/toolsets/{toolset_id}

Response:
{
    "toolset": {
        "id": "uuid",
        "name": "台股分析工具集",
        ...
    },
    "tools": [
        {
            "name": "tej.stock_price",
            "version": "v1",
            "description": "查詢台股股價數據",
            "schema": {...}
        },
        ...
    ]
}
```

### Agent-ToolSet 關聯

#### 1. 分配工具集給 Agent
```http
POST /api/v1/agents/{agent_id}/toolsets
Content-Type: application/json

{
    "toolset_id": "uuid"
}
```

#### 2. 獲取 Agent 的所有工具集
```http
GET /api/v1/agents/{agent_id}/toolsets

Response:
{
    "agent_id": "uuid",
    "agent_name": "財務分析專家",
    "toolsets": [
        {
            "id": "uuid",
            "name": "台股分析工具集",
            "source": "assigned"
        },
        {
            "id": "uuid",
            "name": "全局工具集",
            "source": "global"
        }
    ]
}
```

#### 3. 獲取 Agent 可用的所有工具
```http
GET /api/v1/agents/{agent_id}/available-tools

Response:
[
    {
        "name": "tej.stock_price",
        "version": "v1",
        "description": "...",
        "schema": {...},
        "source": "assigned",
        "toolset_name": "台股分析工具集"
    },
    {
        "name": "searxng.search",
        "version": "v1",
        "description": "...",
        "schema": {...},
        "source": "global",
        "toolset_name": "全局工具集"
    }
]
```

---

## Prompt 生成邏輯

### 舊方式（硬編碼）
```python
user_prompt = f"""
請使用以下工具：
1. tej.stock_price - 查詢股價
2. searxng.search - 網頁搜尋
"""
```

### 新方式（動態生成）
```python
from api.toolset_service import ToolSetService

# 1. 獲取 Agent 可用的工具
available_tools = ToolSetService.get_agent_available_tools(db, agent_id)

# 2. 格式化為 Prompt
tools_prompt = ToolSetService.format_tools_for_prompt(available_tools)

# 3. 注入到 Prompt
user_prompt = f"""
{tools_prompt}

請根據需要選擇合適的工具來完成任務。
"""
```

**生成的 Prompt 範例**：
```
**可用工具**：
1. tej.stock_price (v1) - 查詢台股股價數據
   參數: coid, start_date, end_date, limit
   來源: 台股分析工具集 (assigned)

2. tej.company_info (v1) - 查詢公司基本資料
   參數: coid
   來源: 台股分析工具集 (assigned)

3. searxng.search (v1) - 網頁搜尋
   參數: q, engines
   來源: 全局工具集 (global)

請根據需要選擇合適的工具來完成任務。
```

---

## 前端設計

### 1. 工具集管理頁面

```
┌─────────────────────────────────────────┐
│  工具集管理                              │
├─────────────────────────────────────────┤
│                                         │
│  ┌─ 創建新工具集 ─────────────────┐   │
│  │ 名稱: [___________________]      │   │
│  │ 描述: [___________________]      │   │
│  │                                  │   │
│  │ 選擇工具:                        │   │
│  │ ☑ tej.stock_price               │   │
│  │ ☑ tej.company_info              │   │
│  │ ☐ searxng.search                │   │
│  │ ☐ yfinance.stock_info           │   │
│  │                                  │   │
│  │ ☐ 設為全局工具集                 │   │
│  │                                  │   │
│  │ [創建工具集]                     │   │
│  └──────────────────────────────────┘   │
│                                         │
│  ┌─ 現有工具集 ─────────────────────┐  │
│  │                                  │  │
│  │ 📦 台股分析工具集 (6 個工具)     │  │
│  │    [編輯] [刪除] [查看詳情]      │  │
│  │                                  │  │
│  │ 🌐 全局工具集 (28 個工具)        │  │
│  │    [查看詳情]                    │  │
│  │                                  │  │
│  └──────────────────────────────────┘  │
└─────────────────────────────────────────┘
```

### 2. Agent 管理頁面（工具集分配）

```
┌─────────────────────────────────────────┐
│  Agent: 財務分析專家                     │
├─────────────────────────────────────────┤
│                                         │
│  基本資訊:                              │
│  角色: analyst                          │
│  專長: 台股財務分析                      │
│                                         │
│  ┌─ 分配的工具集 ─────────────────┐   │
│  │                                  │   │
│  │ ✓ 台股分析工具集                 │   │
│  │   6 個工具 | [移除]              │   │
│  │                                  │   │
│  │ ✓ 全局工具集 (自動)              │   │
│  │   28 個工具                      │   │
│  │                                  │   │
│  │ [+ 添加工具集]                   │   │
│  └──────────────────────────────────┘   │
│                                         │
│  ┌─ 可用工具預覽 (34 個) ──────────┐   │
│  │ tej.stock_price                  │   │
│  │ tej.company_info                 │   │
│  │ searxng.search                   │   │
│  │ ...                              │   │
│  └──────────────────────────────────┘   │
└─────────────────────────────────────────┘
```

---

## 實現步驟

### ✅ 已完成
1. 資料庫 Model（ToolSet, AgentToolSet）
2. Pydantic Schemas
3. ToolSetService（業務邏輯）
4. ToolRegistry 擴展（get_tool_info, list_tools）

### 🔄 進行中
5. ToolSet API 端點
6. Agent-ToolSet 關聯 API

### ⏳ 待完成
7. 更新 debate_cycle.py 使用動態工具列表
8. Gradio 前端：工具集管理頁面
9. Gradio 前端：Agent 工具集分配
10. 創建預設工具集（全局、台股分析等）

---

## 使用範例

### 場景 1：創建專門的財務分析 Agent

```python
# 1. 創建台股分析工具集
toolset = {
    "name": "台股分析工具集",
    "tool_names": [
        "tej.stock_price",
        "tej.company_info",
        "tej.monthly_revenue",
        "tej.financial_summary"
    ]
}

# 2. 創建 Agent
agent = {
    "name": "財務分析專家",
    "role": "analyst",
    "specialty": "台股財務分析"
}

# 3. 分配工具集給 Agent
assign_toolset(agent_id, toolset_id)

# 4. Agent 自動獲得工具列表
# - 台股分析工具集（4 個工具）
# - 全局工具集（28 個工具）
# 總共 32 個可用工具
```

### 場景 2：限制 Agent 只能使用特定工具

```python
# 1. 創建受限工具集
restricted_toolset = {
    "name": "僅搜尋工具集",
    "tool_names": ["searxng.search"]
}

# 2. 分配給 Agent
# 3. 如果需要，可以禁用全局工具集（未來功能）
```

---

## 優勢總結

1. **靈活性** - Agent 可以自主選擇工具
2. **可重用性** - 工具集可以分配給多個 Agent
3. **可維護性** - 集中管理工具權限
4. **可擴展性** - 輕鬆添加新工具和工具集
5. **可視化** - 前端清晰展示工具關係
6. **安全性** - 精細控制 Agent 的工具訪問權限

---

## 下一步

1. **立即實現**：ToolSet API 端點（30 分鐘）
2. **短期**：更新 debate_cycle.py（30 分鐘）
3. **中期**：Gradio 前端（2-3 小時）
